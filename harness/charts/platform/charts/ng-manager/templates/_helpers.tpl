...
{{- default (include "ng-manager.name" .) .Values.serviceAccount.name }}
{{- else }}
...
OPA_SERVER_SECRET: {{ .ctx.Values.secrets.default.OPA_SERVER_SECRET | b64enc }}
    ...
    {{- $hasAtleastOneSecret = true }}
HSQS_AUTH_TOKEN: {{ .ctx.Values.secrets.default.HSQS_AUTH_TOKEN | b64enc }}
    {{- end }}
    {{- if eq (include "harnesscommon.secrets.isDefaultAppSecret" (dict "ctx" $ "variableName" "HSQS_AUTH_TOKEN")) "true" }}
    {{- $hasAtleastOneSecret = true }}
...
{{- end }}
{{/*
Manage CGI Executables config
USAGE:
{{- "ng-manager.generate.cgiConfig" .Values.cgiConfig }}
*/}}
{{- define "ng-manager.generate.cgiConfig" }}
    {{- $config := . }}
    {{- $basePath := $config.basePath }}
        {{- range $name, $map := $config.executables }}
            {{- $version := $map.version }}
            {{- $binaryFolderPathOverride := $map.binaryFolderPathOverride }}
  - type: "{{ $name }}"
    version: {{ $version }}
    cgiTaskExecutables:
            {{- range $os, $archs := $map.binaries }}
                {{- range $archs -}}
                    {{- $binaryFolder := default $name $binaryFolderPathOverride }}
                    {{- $value := printf "%s/%s/%s/%s-%s-%s" $basePath $binaryFolder $version $name $os . }}
      - os: {{ $os }}
        arch: {{ . }}
        url: {{ $value }}
                {{- end }}
